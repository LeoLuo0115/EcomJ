...
public class CreateOrderSubscriber implements RocketMQListener<MessageExt> {
    ...
    public void onMessage(MessageExt messageExt) {
        ...
        /**
         * 2 and 3 can combine and send single msg, and have 2 separate listener
         */

        // 2. send "lock-stock" message
        mqRepo.sendMessageToTopic(lockStockTopic, JSON.toJSONString(savedOrderDomain));
        log.info("OrderApp: sent lock-stock message to SQL. OrderId is: " + orderDomain.getOrderNumber());

        // 3. send "pay-check" Delay message, when time is up we need to check whether user have pay order or not
